//! `SkTiledImageUtils`' `DrawImage`/`DrawImageRect` methods are intended to be direct
//! replacements for their [`Canvas`] equivalents. The `SkTiledImageUtils` calls will break
//! `SkBitmap`-backed [`Image`]s into smaller tiles and draw them if the original image is too
//! large to be uploaded to the GPU. If the original image doesn't need tiling or is already
//! gpu-backed the `DrawImage`/`DrawImageRect` calls will fall through to the matching
//! [`Canvas`] call.
...
/// particular, `SkBitmap`-backed [`Image`]s can share a single `SkBitmap` and refer to different
/// subsets of it. In this situation the optimal key is based on the `SkBitmap`'s generation ID
/// and the subset rectangle.
/// For Picture-backed images this method will attempt to generate a concise internally-based
/// key (i.e., containing picture ID, matrix translation, width and height, etc.). For
/// complicated Picture-backed images (i.e., those w/ a paint or a full matrix) it will fall
/// back to using `image`'s unique key.
///
