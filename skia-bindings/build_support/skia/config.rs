...
impl BuildConfiguration {
    pub fn from_features(
        ...
        let env_or_cargo = |name: &str| {
            build_env
                .get(name)
                .map(str::to_string)
                .or_else(|| cargo::env_var(name))
        };

        // Yocto SDKs set CLANGCC/CLANGCXX, which is a better choice to determine clang,
        // as CC/CXX are likely referring to gcc.
        let cc = env_or_cargo("CLANGCC")
            .or_else(|| env_or_cargo("CC"))
            .unwrap_or_else(|| "clang".to_string());
        let cxx = env_or_cargo("CLANGCXX")
            .or_else(|| env_or_cargo("CXX"))
            .unwrap_or_else(|| "clang++".to_string());
...
pub fn build(
    ...
    configure_skia(build, config, build_env, python, gn_command.as_deref());
    build_skia(config, build_env, &ninja);
}
